...
public class NetworkUtils {
    ...
    public static WifiDetails getWifiDetails(Context context) {
        WifiDetails wifiDetails = new NetworkUtils.WifiDetails();
        wifiDetails.ssid = "local";
        wifiDetails.ipAddress = "127.0.0.1";
                ...
                if (intf.getName().contains("wlan")) {
                    for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements(); ) {
                        ...
                        if (!inetAddress.isLoopbackAddress() && (inetAddress.getAddress().length == 4)) {
                            GLog.d(LOG_TAG, inetAddress.getHostAddress());
