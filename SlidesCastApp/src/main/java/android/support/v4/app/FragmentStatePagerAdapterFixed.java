...
public abstract class FragmentStatePagerAdapterFixed extends PagerAdapter {
	...
	public FragmentStatePagerAdapterFixed(FragmentManager fm) {
		mFragmentManager = fm;
	}
	...
	public Object instantiateItem(ViewGroup container, int position) {
		...
		if (DEBUG)
			GLog.v(TAG, "Adding item #" + position + ": f=" + fragment);
		...
		Fragment previousFragment = retrieveFragment(fragment.getClass());
		if (previousFragment == null) {
			mCurTransaction.add(container.getId(), fragment);
		} else {
			mCurTransaction.show(previousFragment);
		}
		return fragment;
	}
	...
	public void destroyItem(ViewGroup container, int position, Object object) {
		...
		if (DEBUG)
			GLog.v(TAG, "Removing item #" + position + ": f=" + object + " v=" + ((Fragment) object).getView());
			...
			for (String key : keys) {
				if (key.startsWith("f")) {
					...
					if (f != null) {
						...
						GLog.w(TAG, "Bad fragment at key " + key);
					}
				}
			}
		}
