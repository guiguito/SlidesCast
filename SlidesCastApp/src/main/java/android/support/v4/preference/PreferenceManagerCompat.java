...
public class PreferenceManagerCompat {
	...
	static PreferenceManager newInstance(Activity activity, int firstRequestCode) {
		try {
			...
			GLog.w(TAG, "Couldn't call constructor PreferenceManager by reflection" + e.getMessage());
		}
		return null;
	}
	...
	static void setOnPreferenceTreeClickListener(PreferenceManager manager, final OnPreferenceTreeClickListener listener) {
		try {
			...
			GLog.w(TAG, "Couldn't set PreferenceManager.mOnPreferenceTreeClickListener by reflection" + e.getMessage());
		}
	}
	...
	static PreferenceScreen inflateFromIntent(PreferenceManager manager, Intent intent, PreferenceScreen screen) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.inflateFromIntent by reflection" + e.getMessage());
		}
		return null;
	}
	...
	static PreferenceScreen inflateFromResource(PreferenceManager manager, Activity activity, int resId, PreferenceScreen screen) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.inflateFromResource by reflection" + e.getMessage());
		}
		return null;
	}
	...
	static PreferenceScreen getPreferenceScreen(PreferenceManager manager) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.getPreferenceScreen by reflection" + e.getMessage());
		}
		return null;
	}
	...
	static void dispatchActivityResult(PreferenceManager manager, int requestCode, int resultCode, Intent data) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.dispatchActivityResult by reflection " + e.getMessage());
		}
	}
	...
	static void dispatchActivityStop(PreferenceManager manager) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.dispatchActivityStop by reflection" + e.getMessage());
		}
	}
	...
	static void dispatchActivityDestroy(PreferenceManager manager) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.dispatchActivityDestroy by reflection" + e.getMessage());
		}
	}
	...
	static boolean setPreferences(PreferenceManager manager, PreferenceScreen screen) {
		try {
			...
			GLog.w(TAG, "Couldn't call PreferenceManager.setPreferences by reflection " + e.getMessage());
		}
		return false;
	}
