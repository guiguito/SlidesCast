...
//handle postmessage events	
	function receiveMessage(event)
	{
	   if (event.data == 'first'){
		...
		var tabs = viewer.getPages();
		viewer.showPage(tabs.length);
	  }else if (event.data == 'next'){
		viewer.showNextPage();
	  }else if (event.data == 'previous'){
		viewer.showPreviousPage();
	  }else if (event.data.substring(0, 6) == 'jumpto'){
		var page = event.data.substring(7,event.data.length);
		page = page-1;	;
